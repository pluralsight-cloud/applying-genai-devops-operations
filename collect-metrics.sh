...
# Strip any trailing slash so we don't build URLs with a double slash
# (e.g. ".../:9090//api/v1"), which Prometheus answers with a 307 redirect.
PROMETHEUS_URL="${PROMETHEUS_URL%/}"
BASE="$PROMETHEUS_URL/api/v1"

echo "Verifying Prometheus is reachable..."
curl -sfL "$PROMETHEUS_URL/-/healthy" > /dev/null \
  || { echo "ERROR: Prometheus not reachable at $PROMETHEUS_URL"; exit 1; }

echo "Exporting RPS time series (7 days, 1h resolution)..."
curl -sfGL "$BASE/query_range" \
...
curl -sfGL "$BASE/query" \
  --data-urlencode "query=min_over_time((sum(rate(http_requests_total{job=\"$APP_LABEL\"}[5m])) > 0.1)[7d:5m])" \
