...
Requires [Docker](https://docs.docker.com/get-docker/) on your `PATH` (for example Docker Desktop on Windows).

Build image from this directory (`app/`):

```bash
docker build -t payment-api:local .
```

Optional build-time port (default `3000`; image `HEALTHCHECK` still targets port `3000` per course spec):

```bash
docker build -t payment-api:local --build-arg PORT=3000 .
```
...
This repo intentionally contains planted issues for instructional use (clearly tagged in source comments).
